export const PARADIGM_PATTERNS = [
    {
        id: "strangler-facade",
        ...
        assumptionBroken: "Modernization must replace the entire system before anything ships.",
        approachTemplate: "Stand up a thin facade that owns one workflow touching {painStatement}. Route new consumers to the facade while the legacy core stays in place behind it. Migrate read or write paths incrementally with explicit rollback switches.",
    ...
    },
    {
        ...
        assumptionBroken: "You must refactor the database before any digital channel can access legacy data.",
        approachTemplate: "Document each screen field and transaction sequence tied to {painStatement}. Expose a read or command API that mirrors the terminal flow without schema surgery. Pilot with one downstream consumer before widening access.",
    ...
    },
    {
        ...
        assumptionBroken: "You can only validate a migration in a one-shot production cutover.",
        approachTemplate: "Replay or mirror traffic for {painStatement} through a parallel path. Compare outputs, latency, and side effects daily. Cut over only when diffs stay within agreed thresholds for a full business cycle.",
        scopeDefault: "3 weeks",
        risks: [
    ...
    },
    {
        ...
        assumptionBroken: "Teams must start coding before anyone documents what the legacy system actually does.",
        approachTemplate: "Interview operators and trace {painStatement} through tables, jobs, and integrations. Produce a cited map of entities, owners, and blast radius. End with three scoped cutover options leadership can fund.",
        scopeDefault: "2 weeks",
        risks: [
    ...
    },
    {
        ...
        assumptionBroken: "Reporting modernization must wait until the transactional core is replaced.",
        approachTemplate: "Extract the datasets behind {painStatement} into a governed analytics layer. Ship the reports leadership actually asks for while the transactional system keeps running. Defer core replacement until read pain is solved.",
        scopeDefault: "3 weeks",
        risks: [
    ...
    },
    {
        ...
        assumptionBroken: "Slow reads require rewriting application code or replacing the database.",
        approachTemplate: "Target the specific access pattern behind {painStatement}. Add indexes, materialized views, or a read replica dedicated to that workload. Keep writes on the legacy path until a separate initiative owns them.",
        scopeDefault: "2 weeks",
        risks: [
    ...
    },
    {
        ...
        assumptionBroken: "Partial migrations always leak into adjacent modules and never finish.",
        approachTemplate: "Draw a hard boundary around the workflow tied to {painStatement}. Migrate that module's reads, writes, and integrations as one package. Adjacent modules stay on legacy interfaces with documented stubs.",
    ...
    },
    {
        ...
        assumptionBroken: "Legacy knowledge must be manually re-read every time someone asks a question.",
        approachTemplate: "Ingest the document residue behind {painStatement}. Chunk, index, and cite sources so operators can answer audit or ops questions in minutes. Keep human review on high-risk citations before external use.",
        scopeDefault: "2 weeks",
        risks: [
